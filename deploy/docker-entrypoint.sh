...
# - По умолчанию UVICORN_HOST=:: (dual-stack на Linux): после «Recreating container with
#   healthcheck» встроенная проба часто ходит на http://localhost:PORT/…; в контейнере
#   localhost может резолвиться в ::1. При привязке только 0.0.0.0 на ::1 никто не слушает
#   → healthcheck падает, хотя с 172.18.x.x (IPv4) HEAD / уже 200. Uvicorn с --host :: на
#   Linux принимает и IPv4, и IPv6 (IPV6_V6ONLY=0).
