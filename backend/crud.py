...
from bot import send_telegram_message, escape_markdown
...
async def create_transaction(db: AsyncSession, tr: schemas.TransferRequest):
    ...
    try:
        message_text = (f"🎉 Вам начислена *1* спасибка!\n"
                        f"От: *{escape_markdown(sender.first_name or '')} {escape_markdown(sender.last_name or '')}*\n"
                        f"Сообщение: _{escape_markdown(tr.message or '')}_")
        # Игнорируем анонимизированных пользователей (telegram_id < 0)
...
async def create_purchase(db: AsyncSession, pr: schemas.PurchaseRequest):
    ...
    try:
        ...
        admin_message = (
            ...
            f"👤 *Пользователь:* {escape_markdown(user.first_name or '')} (@{escape_markdown(user.username or str(user.telegram_id))})\n"
            f"💼 *Должность:* {escape_markdown(user.position or '')}\n\n"
            f"🎁 *Товар:* {escape_markdown(item.name)}\n"
        ...
        user_message = f"🎉 Поздравляем с покупкой \"{escape_markdown(item.name)}\"!"
        if issued_code_value:
            # Для товаров с кодом добавляем сам код
            user_message += f"\n\nВаш уникальный код/ссылка:\n`{escape_markdown(issued_code_value)}`"
...
async def process_birthday_bonuses(db: AsyncSession):
    ...
    for user in users:
        ...
        if user.telegram_id and user.telegram_id >= 0 and user.status == "approved":
            birthday_message = (
                f"🎉 *С Днем Рождения!* 🎂\n\n"
                f"Дорогой/ая *{escape_markdown(user.first_name or 'коллега')}*, поздравляем вас с днем рождения!\n\n"
...
async def request_profile_update(db: AsyncSession, user: models.User, update_data: schemas.ProfileUpdateRequest):
    ...
    message_lines = [
        f"👤 *Запрос на смену данных от:* @{escape_markdown(user.username or user.first_name or '')} ({escape_markdown(user.last_name or '')})\n"
    ]
    
    for key, new_val in actual_new_data.items():
        old_val = old_data.get(key)
        field_name = key.replace('_', ' ').capitalize()
        old_val_str = escape_markdown(str(old_val)) if old_val else 'не указано'
        new_val_str = escape_markdown(str(new_val)) if new_val else 'не указано'
...
async def admin_update_user(db: AsyncSession, user_id: int, user_data: schemas.AdminUserUpdate, admin_user: models.User):
    ...
    if changes_log:
        ...
        log_message = (
            ...
            f"👤 *Администратор:* {escape_markdown(admin_name)}\n"
            f"🎯 *Пользователь:* {escape_markdown(target_user_name)}\n\n"
            f"*Изменения:*\n" + "\n".join([escape_markdown(change) for change in changes_log])
        )
...
async def admin_delete_user(db: AsyncSession, user_id: int, admin_user: models.User):
    ...
    log_message = (
        ...
        f"👤 *Администратор:* {escape_markdown(admin_name)} (`{admin_user.id}`)\n"
        f"🎯 *Бывший пользователь:* {escape_markdown(target_user_name)} (`{user_id}`)\n\n"
    ...
    try:
        ...
        if invited_user.telegram_id and invited_user.telegram_id >= 0:
            await send_telegram_message(
                ...
                f"👤 *{escape_markdown(buyer.first_name or '')} {escape_markdown(buyer.last_name or '')}* приглашает вас разделить товар *{escape_markdown(item.name)}*\n\n"
...
async def accept_shared_gift_invitation(db: AsyncSession, invitation_id: int, user_id: int):
    ...
    try:
        ...
        if buyer.telegram_id and buyer.telegram_id >= 0:
            await send_telegram_message(
                ...
                f"👤 *{escape_markdown(invitation.invited_user.first_name or '')} {escape_markdown(invitation.invited_user.last_name or '')}* согласился разделить товар *{escape_markdown(item.name)}*\n\n"
    ...
    try:
        admin_message = (
            ...
            f"👤 *Покупатель:* {escape_markdown(buyer.first_name or '')} {escape_markdown(buyer.last_name or '')} (@{escape_markdown(buyer.username or str(buyer.telegram_id))})\n"
            f"👥 *Приглашенный:* {escape_markdown(invitation.invited_user.first_name or '')} {escape_markdown(invitation.invited_user.last_name or '')} (@{escape_markdown(invitation.invited_user.username or str(invitation.invited_user.telegram_id))})\n\n"
            f"🎁 *Товар:* {escape_markdown(item.name)}\n"
        ...
        if buyer and item:
            ...
            if buyer.telegram_id and buyer.telegram_id >= 0:
                await send_telegram_message(
                    ...
                    f"👤 *{escape_markdown(invitation.invited_user.first_name or '')} {escape_markdown(invitation.invited_user.last_name or '')}* отклонил приглашение на товар *{escape_markdown(item.name)}*\n\n"
