...
def _send_fcm_sync(token: str, payload: dict[str, Any]) -> tuple[bool, bool]:
    """Returns (delivered, deactivate_token)."""
    if not _ensure_firebase_app():
        return False, False

    title = str(payload.get("title") or "Спасибо")
    body = str(payload.get("body") or "")
    url = str(payload.get("url") or "/")
    tag = str(payload.get("tag") or "spasibo")
...
async def send_user_fcm_push(
    ...
    *,
    ...
    payload = {
        ...
        "tag": tag or f"spasibo-android-{user_id}-{int(datetime.utcnow().timestamp() * 1000)}",
    }
